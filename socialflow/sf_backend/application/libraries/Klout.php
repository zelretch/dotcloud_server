...
class Klout {
    ...
    private $api_url = 'http://api.klout.com/v2/';
    private $api_key;
	...
	public function get_id($uri, $params = array(), $format = NULL) {
        if (!empty($params)) {
            $uri .= '.'  . $format . '/twitter?' . 'key=' . $this->api_key . '&' . http_build_query($params);
        }
        return $this->_call('get', $uri, NULL, $format);
    }
	public function get_score($uri, $id, $format = NULL) {
        
        $uri .= '.'  . $format . '/' . $id . '/score?' . 'key=' . $this->api_key ;
